...
export class Book {
  ...
  static findOneAndUpdate(
    ...
    const updatedBookDocArr = bookDocArr.map((bookDoc, index) =>
      String(index) === String(bookIndex)
        ? { ...bookDoc, ...bookData }
        : bookDoc,
    )

    //!! update bookDocArr into localStorage
    storage.saveToLocalStorage('bookStorage', updatedBookDocArr)

    console.log(
      '%cĐã update Book:',
      'color: yellow;',
      JSON.stringify(updatedBookDocArr[bookIndex]),
      '\n',
    )
    return updatedBookDocArr[bookIndex]
  }
